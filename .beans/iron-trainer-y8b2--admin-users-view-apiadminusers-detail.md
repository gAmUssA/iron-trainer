...
## Todo
- [ ] GET /api/admin/users → all athletes: id, name, strava_athlete_id, apple linked?, connected?, ftp/thresholds set?, counts (activities, jobs, recent-failures).
- [ ] GET /api/admin/users/{id} → detail: connected accounts (Strava/Apple), last-sync-per-kind, recent jobs, data counts (activities, daily_recovery days).
- [ ] Frontend: Users table + a user-detail drawer/page (accounts, last sync, recent jobs).
## Notes
