...
## Todo
- [x] GET /api/admin/users → all athletes: id, name, strava_athlete_id, apple_linked, connected, and counts (activities, jobs, all-time failed_jobs). (Thresholds live on the detail endpoint, not the list; failed_jobs is an all-time count, not recent-only.)
