...
Strangler note: bearer-authenticated GETs for the endpoints backend-v2 owns are
forwarded to it by app.strangler.StranglerProxyMiddleware (config-driven path
allowlist) — the handlers below run only when the request is served locally
(flip off, cookie auth, path not in the allowlist, or backend-v2 unreachable).
The zip bundles are Python-only and never proxied.
"""
